...
public class CartControllerTest {
	...
	public void addToCartTest() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/cart/add").param("quantity", "1").param("productId", "1"))
				...
				.andExpect(flash().attributeExists("flash")); //check for FlashMessage
...
// adding too many throws NotEnoughProductAvailableException
	@Test
	public void addTooMany_ShouldThrowNotEnoughProductAvailableException() throws Exception{
			...
			when(productService.isEnoughProductInStock(product,q)).thenThrow(NotEnoughProductAvailableException.class);


			mockMvc.perform(MockMvcRequestBuilders.post("/cart/add").param("quantity",String.valueOf(q)).param("productId", "1").header("referer", "/product/"))
					.andDo(print())
					.andExpect(status().is3xxRedirection())
					.andExpect(redirectedUrl("/product/"))
	...
	// updating with too many throws NotEnoughProductAvailableException
	@Test
	public void updateTooMany_ShouldThrowNotEnoughProductAvailableException() throws Exception {
		Product product = productBuilder();
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/cart/update").param("newQuantity", String.valueOf(q)).param("productId", "1").header("referer", "/product/"))
				.andDo(print())
				.andExpect(status().is3xxRedirection())
				.andExpect(redirectedUrl("/product/"))
				.andExpect(flash().attribute("flash", Matchers.hasProperty("status", Matchers.equalTo(FlashMessage.Status.FAILURE))));
