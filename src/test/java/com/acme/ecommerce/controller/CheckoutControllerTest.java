...
public class CheckoutControllerTest {
	...
	//validating CouponCode
	@Test
	public void validCouponCode_ShouldBeMoreThan4Characters() throws Exception {
			mockMvc.perform(MockMvcRequestBuilders.post("/checkout/coupon").param("code", "12")).andDo(print())
					.andExpect(status().is3xxRedirection())
					.andExpect(flash().attribute("org.springframework.validation.BindingResult.couponCode",hasProperty("fieldErrorCount", equalTo(1))))
					.andExpect(redirectedUrl("coupon")
					);
	}

	@Test
	public void validCouponCode_ShouldBeLessThan11Characters() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post("/checkout/coupon").param("code", "12345678910")).andDo(print())
				.andExpect(status().is3xxRedirection())
				.andExpect(flash().attribute("org.springframework.validation.BindingResult.couponCode",hasProperty("fieldErrorCount", equalTo(1))))
				.andExpect(redirectedUrl("coupon")
				);
	...
	@Test
	public void validCouponCode_NoCouponCodeIsOK() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post("/checkout/coupon").param("code", "")).andDo(print())
				.andExpect(status().is3xxRedirection())
				.andExpect(redirectedUrl("shipping"));
	}
