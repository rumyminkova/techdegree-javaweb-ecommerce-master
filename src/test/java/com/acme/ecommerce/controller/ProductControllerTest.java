...
public class ProductControllerTest {
	...
	public void getIndex() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/product/"))
		    ...
		    .andExpect(model().attributeExists("subtotalFormatted"));   //Check if subTotal is added
	}
...
//show error page if product not found
	@Test
	public void product_ShouldErrorOnNotFound() throws Exception{
		when(productService.findById(1L)).thenThrow(ProductNotFoundException.class);

		mockMvc.perform(MockMvcRequestBuilders.get("/product/detail/1"))
				.andDo(print())
				.andExpect(view().name("error"))
				.andExpect(model().attribute("ex", org.hamcrest.Matchers.instanceOf(ProductNotFoundException.class)));
